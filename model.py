...
class MiT(nn.Module):
    def __init__(
        ...
        *,
        ...
        for (dim_in, dim_out), (kernel, stride, padding), num_layers, ff_expansion, heads, reduction_ratio in zip(
            ...
            embedding = Embedding(embed_dim = dim_in, kernel_size = kernel, stride = stride, padding = padding)
    ...
    def forward(
        ...
        layer_outputs = []
        for (embedding, layers) in self.stages:
            x = embedding(x)
...
class ARCFormer(nn.modules):
    def __init__(
        ...
        *,
        ...
        decoder_dim=256,
        num_classes=19,
        kernel_stride_padding=((7, 4, 3), (3, 2, 1), (3, 2, 1), (3, 2, 1))
        ):
        super().__init__()
        ...
        self.mit = MiT(
            ...
            stage_kernel_stride_pad=kernel_stride_padding
        )
        # Decoder를 통해 attention을 적용하여 결합
        self.decoder = Decoder(gate_channels = dims[-1] * len(kernel_stride_padding))  
        self.head = Head(input_dim=dims[-1] * len(kernel_stride_padding), dim=dims[-1], num_classes=num_classes)
...
if __name__ == '__main__':
    ...
    model = ARCFormer(**model_args).to(device)
